...
export default function Header() {
  const isScrolled = useScrollEffect(5, 80); // Immediate scroll down, minimal delay up
  ...
  return (
    <>
      <header
        className={`
          fixed z-50 text-main-black transition-all duration-200 ease-out
          ${isAdminDashboard 
            ? 'top-0 left-0 right-0 bg-main-white h-[70px] px-[15px] sm:px-5 md:px-[30px]'
            : isScrolled 
              ? 'top-4 left-1/2 -translate-x-1/2 bg-main-white h-[60px] rounded-2xl shadow-lg backdrop-blur-md w-[calc(100vw-30px)] sm:w-[calc(100vw-40px)] md:w-[calc(100vw-60px)] max-w-[1300px] 2xl:max-w-[1650px] px-[5px] sm:px-1 md:px-[20px]'
              : 'top-0 left-1/2 -translate-x-1/2 bg-transparent h-[90px] w-[calc(100vw-30px)] sm:w-[calc(100vw-40px)] md:w-[calc(100vw-60px)] max-w-[1300px] 2xl:max-w-[1650px] px-[15px] sm:px-5 md:px-[30px]'
          }
        `}
        role="banner"
        itemScope
      ...
      >
        <div className="relative w-full max-w-[1300px] 2xl:max-w-[1650px] mx-auto h-full flex items-center justify-between">
          ...
          <div className={`lg:absolute lg:left-1/2 lg:top-1/2 lg:-translate-x-1/2 lg:-translate-y-1/2 ${isScrolled ? 'pl-2 sm:pl-3 md:pl-4' : ''}`}>
            <Link href="/" aria-label="X2M Creative - Vai alla homepage">
              <Image
                ...
                className={`transition-transform duration-200 ease-out ${
                  isAdminDashboard ? 'scale-90' : isScrolled ? 'scale-90' : 'scale-100'
                }`}
          ...
          {!isClient || !isMobileMenuOpen ? (
            <button 
              className={`p-2 lg:hidden ml-auto transition-colors duration-300 relative z-[1001] text-current ${isScrolled ? 'mr-2 sm:mr-3 md:mr-4' : ''}`}
